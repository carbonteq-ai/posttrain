...
These percentages do not add to 40% of compute. Each discovery slot also makes a class choice. That choice may follow either the class prediction or the class coverage component. Class exploration is a probability mixture; discovery is a counted allocation. Neither is a promise about the composition of the retained optimizer batch.

### 7.2 What class evidence predicts

An all-success group and an all-failure group both give zero relative outcome advantages. The useful prediction is the probability of a mixed group under the current student.

Let $p$ be a task's success probability and $G$ the fixed number of fresh attempts. Under conditionally independent binary attempts,

$$
m_G(p)=1-p^G-(1-p)^G.
$$

A class contains tasks with different success probabilities. Let $\mathcal F_c(p)$ describe the estimated distribution of those probabilities for a new task from class $c$. Its predicted mixed-group rate is

$$
u_c^{\mathrm{new}}=\mathbb E_{p\sim \mathcal F_c}[m_G(p)].
$$

Suppose 60% of a class's tasks always succeed and the other 40% succeed half the time. With four attempts, the predicted mixed-group rate for a new task is

$$
u_c^{\mathrm{new}}=0.60(0)+0.40(0.875)=0.35.
$$

The 60% figure reduces the opportunity; it does not exhaust the class. If 90% always succeed and 10% succeed half the time, the rate falls to 0.0875. These are idealized populations illustrating the calculation, not estimates from a few observed groups.

The first population has average success 0.8. Substituting that mean into $m_4$ gives 0.5888, which is different from 0.35. Predict the distribution of task behavior rather than treating every task as an average task. The corresponding expected within-task variance is $\mathbb E_{\mathcal F_c}[p(1-p)]$, also different from the variance computed from the class mean.

Representative discovery observations and reference probes can inform $\mathcal F_c$. Repeated exploitation of a few selected tasks must not dominate it. Record the sampling design, distinct-task and source-family coverage, uncertainty, and model age. Uniform task draws within a class's unseen pool form a randomized discovery order; they are not fresh reference measurements of tasks already used in training. Transfer, changing model weights, and depletion of that pool can change how well earlier discoveries predict the remaining tasks.

When class evidence is sparse or unrepresentative, shrink toward a shared weak prior or use the base selection fallback. The fitted distribution and its calibration are part of the versioned evidence policy. The class label alone cannot justify a confident prediction.

### 7.3 A task corrects its class prediction

Before its first attempt, a task uses the class distribution as a prior. As fresh task outcomes accumulate, they provide increasingly specific evidence. One candidate formulation updates a class prior using $n_i^+$ successes and $n_i^-$ failures for task $i$:

$$
d\mathcal F_i(p)=
\frac{p^{n_i^+}(1-p)^{n_i^-}\,d\mathcal F_{c,-i}(p)}
{\int_0^1 x^{n_i^+}(1-x)^{n_i^-}\,d\mathcal F_{c,-i}(x)}.
$$

Here $\mathcal F_{c,-i}$ excludes the task's own evidence, or is frozen before that evidence is collected, to avoid counting the same outcomes twice. Keep support inside $(0,1)$ so an initial class belief cannot make contrary outcomes impossible. A beta prior or a mixture of beta distributions can supply that support; a mixture can represent a class containing both consistently solved and mixed-outcome tasks.

The task's practice score is

$$
u_i=\mathbb E_{p\sim \mathcal F_i}[m_G(p)].
$$

At zero observations, this equals the class prediction. With enough compatible observations, the task's own behavior dominates. Repeated success pushes its predicted mixed-group rate toward zero; observed failures can restore it.

This posterior equation assumes a stable task success probability over the included observations and an appropriate independence model. A changing student violates indefinite pooling. Use bounded recent evidence with checkpoint identities; reset or discount incompatible history under an explicit evidence policy. Discounted or cross-checkpoint updates are tracking approximations, not exact stationary posteriors. Evidence age alone never establishes task coverage.

Predicting mixed groups is a proxy for available outcome-relative signal. It does not estimate causal learning gain, and verifier noise can produce high scores. Predictive calibration and held-out progress must determine whether this extra machinery improves on direct recent variance estimates.
...
Select reserved discovery slots first, then fill the remaining request. Discovery draws use unseen tasks outside $S_k$. Other slots use previously selected tasks outside $S_k$; if that pool runs out, use additional unseen tasks and record the fallback. Recompute eligibility after each draw.

Class predictions direct discovery toward promising remaining inventory. For ordinary practice, class scores summarize the predicted contrast of its eligible familiar tasks using fixed task weights, so repeated visits do not multiply a task's influence. These are predictions for the current candidate pool, not unbiased class-performance reports.

For a draw, let $E$ be its eligible task pool, $\mathcal C_E$ the classes with tasks in that pool, and $b_E(c)$ the base class weights renormalized over $\mathcal C_E$. Let $a_c(E)$ be $u_c^{\mathrm{new}}$ for discovery, or the weighted mean of $u_i$ over eligible familiar tasks for ordinary practice. Define

$$
A_E(c)=\frac{b_E(c)a_c(E)}
{\sum_{d\in\mathcal C_E}b_E(d)a_d(E)},\qquad
Q(c\mid E)=\epsilon_c b_E(c)+(1-\epsilon_c)A_E(c).
$$

When all scores are zero, or the evidence policy cannot supply comparable scores, use $A_E=b_E$. Draw from this mixture with randomized categorical selection; do not repeatedly round tiny class allocations down to zero.
...
Within a selected class, let $R_c$ be the reassessment distribution and $W_c$ the adaptive task distribution. For an ordinary-practice draw, the actual joint law is

$$
Q(i\mid E)=
\epsilon_c b_E(c(i))R_{c(i)}(i)
+(1-\epsilon_c)A_E(c(i))W_{c(i)}(i).
$$

For discovery, replace both within-class distributions by the base distribution restricted to unseen tasks. The coverage and adaptive components can choose different task distributions, so multiplying a marginal class share by one unconditional task share would misreport the selection probability.

The class floor is conditional: $Q(c\mid E)\ge\epsilon_c b_E(c)$ for eligible classes. A class with no unseen tasks cannot receive a discovery slot. Its ordinary-practice eligibility can still support reassessment. Discovery constraints and step exclusions change the overall class mixture; the original unrestricted floor is not a guarantee for every realized batch. Log eligible pools and conditional probabilities before each draw.
...
Consider arithmetic and algebra with equal base class weights. Suppose representative class models predict mixed-group rates of 0.35 and 0.70 for unseen tasks. At 20% class exploration,

$$
Q(\mathrm{arithmetic}\mid E_{\mathrm{new}})
=0.2(0.5)+0.8\frac{0.35}{0.35+0.70}
\approx0.367.
$$

Algebra receives approximately 0.633. If arithmetic's predicted rate later falls to 0.0875 while algebra stays at 0.70, arithmetic's discovery share falls to approximately 0.189. It still receives some discovery because its class forecast may miss useful tasks.
...
Arithmetic task A17 inherits a low mixed-group forecast from its class. Discovery selects it anyway through the remaining class probability. Its answers include both successes and failures.
...
For the illustrative estimator, a class prior averages one first-discovery group per distinct task from the last eight steps, with 20% weak-prior smoothing on a 41-point probability grid. Each task applies up to four recent groups to a prior excluding its own discovery evidence. This smoothing parameter is unrelated to either exploration budget. The estimator is a concrete tracking approximation for the visual demonstration, not a calibrated production recommendation. Expiring class evidence can restore uncertainty even when old results were consistently successful.
