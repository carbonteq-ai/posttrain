...
def test_soft_selector_reports_shortfall_when_distinct_pool_cannot_fill_refill():
    groups = [
        {"task_id": str(index), "class": "x", "rewards": [0, 0, 0, 0], "input_tokens": 100, "output_tokens": 20}
        for index in range(10)
    ]
    result = run_simple(groups, "soft_yield_explore", 3, settings=Settings(stability_penalty=0, uncertainty_weight=0.2))
