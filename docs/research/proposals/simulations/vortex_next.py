...
class Sampler:
    ...
    def scores(self, step):
        ...
        for task, cls in self.inventory.items():
            ...
            peer_n, peer_first, peer_second = (
                ...
                for index, total in enumerate(classes[cls])
            )
    ...
    def probabilities(self, eligible, scores, explore):
        ...
        class_weights = normalize({
            cls: statistics.mean(task_weights[t] for t in tasks)
...
class World:
    ...
    def group(self, task, step):
        ...
        pool = own if int.from_bytes(digest[:8], 'big') / 2**64 < len(own) / (len(own) + self.shrinkage) else self.peers[task]
...
def run(groups, policy, seed, scenario='stationary', settings=DEFAULT_SETTINGS, shrinkage=4.0):
    ...
    return {
        'policy': policy, 'seed': seed, 'scenario': scenario,
        **dict(counts), 'covered_tasks': len(seen),
...
def experiment(snapshot, seeds=10):
    ...
    for key in sorted({(r['source'], r['scenario'], r['policy']) for r in rows}):
        ...
        for metric in ['completed_steps', 'candidates_per_retained', 'zero_variance_fraction',
                       'mean_rounds', 'new_fraction', 'late_new_fraction', 'covered_tasks', 'mean_signal',
                       'mastered_constant', 'low_constant']:
    ...
    return {'settings': asdict(Settings()), 'seeds': seeds, 'support': support,
