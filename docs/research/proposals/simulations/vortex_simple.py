...
POLICIES = ('current_controller', 'binary_soft20', 'soft_yield', 'soft_yield_explore',
            'production_yield_first')
...
def score_tasks(controller, sampler, step):
    ...
    return {
        task: (controller.task_priority(task), uncertainty[task][1])
        for task in sampler.inventory
    }
...
def run_simple(groups, policy, seed, scenario='stationary', settings=BASE_SETTINGS):
    ...
    controller_settings = (
        AdaptiveCurriculum(
            'class', seed=seed, policy='yield_first',
        ...
        if policy == 'production_yield_first' else AdaptiveCurriculum('class', seed=seed)
    )
    ...
    return {
        'policy': policy, 'seed': seed, 'scenario': scenario, **dict(counts),
...
def experiment(snapshot, seeds=10):
    ...
    for source in snapshot['runs']:
        ...
        for scenario in ('stationary', 'saturation_recovery'):
            for policy in POLICIES:
                settings = replace(
                    BASE_SETTINGS,
                    uncertainty_weight=(1.0 if policy == 'binary_soft20' else
                                        0.2 if policy == 'soft_yield_explore' else
                                        4.0 if policy == 'production_yield_first' else 0),
                )
    ...
    for key in keys:
        ...
        metrics = {}
        for metric in ('completed_steps', 'candidates_per_retained', 'zero_variance_fraction',
                       'mean_rounds', 'new_fraction', 'late_new_fraction', 'covered_tasks',
                       'mastered_constant', 'low_constant', 'mean_std', 'mean_variance',
    ...
    return {'settings': asdict(BASE_SETTINGS), 'seeds': seeds, 'support': support,
...
def main():
    ...
    result['previous_simulator_sha256'] = hashlib.sha256(Path(__file__).with_name('vortex_next.py').read_bytes()).hexdigest()
    result['controller_sha256'] = hashlib.sha256(Path(
        'packages/train/src/posttrain/train/adaptive_curriculum.py'
