...
assert.equal(E.mixed(0.5), 0.875);
assert.equal(E.mixed(0), 0);
assert.equal(E.mixed(1), 0);
...
const uniform = E.GRID.map(() => 1 / E.GRID.length);
const unknown = E.forecast(uniform);
check(E.forecast(E.posterior(uniform, 8, 8)) > unknown, "Mixed task evidence increases the forecast");
check(E.forecast(E.posterior(uniform, 16, 0)) < unknown, "Repeated success reduces forecast");
const tasks = E.inventory();
tasks[0].discovery = {step: 1, success: 4};
const before = E.classPrior(tasks, 0, 1);
tasks[0].history = Array.from({length: 100}, () => ({step: 1, success: 4}));
assert.deepEqual(E.classPrior(tasks, 0, 1), before, "Exploitation visits cannot multiply class evidence");
assert.deepEqual(E.classPrior(tasks, 0, 10), uniform, "Old class observations expire");
...
const a = E.choose(tasks, scores, new Set(), true, 20, E.random(5));
const polluted = tasks.map(t => ({...t, hiddenP: 1, truth: 0, rate: 999}));
const b = E.choose(polluted, scores, new Set(), true, 20, E.random(5));
assert.deepEqual(a, b);
tasks.forEach((t, i) => { t.seen = true; t.last = i; });
const zeroScores = {classes: [0, 0, 0, 0], tasks: tasks.map(() => 0)};
const recheck = E.choose(tasks, zeroScores, new Set(), false, 0, E.random(3));
...
if (process.argv.includes("--dom")) {
  ...
  for (const file of ["task-discovery-v2.html", "../configurable-curriculum-and-data-preparation-v2.html"]) {
    ...
    assert.equal(d.querySelectorAll(".td-success-point").length, 4);
    el("next").click();
    ...
    assert.equal(d.querySelector('[data-lane="discovery"]').dataset.count, "2");
