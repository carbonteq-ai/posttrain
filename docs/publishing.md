...
and images without a merged, CI-green commit have no reviewable source. The
GitHub protocol below is part of the release, not an afterthought.
...
(or briefly on GHCR if you staged there).
...
Do this **before** uploading wheels or cutting the tag. Publishing is
irreversible; merging a broken tree is how irreversible mistakes get a SHA.
...
2. **If images were republished, commit `published.toml` on the same branch.**
   The manifest is generated by `posttrain-release images publish`; hand edits
   are how digests and locks previously drifted. Confirm
   `framework_version` and that workspace-lock images show the new
   `lock_digest` before committing.
3. **Push the branch** (`git push -u origin HEAD` when the upstream is new).
4. **Wait for CI green on that push.** The last green check on an older SHA
...
5. **Merge the pull request** only when CI is green on the head you just
   pushed. Prefer the repository’s normal merge method (`gh pr merge` with
   the project’s required checks). Do not merge from a dirty or divergent
   local tree.
6. **Only then cut the release** (distributions, tag, any remaining image
   mirror). A version on the index whose tree is not on `main` (or whatever
   the default branch is) has no reviewable source.
...
6. **If the constraint lock changed, republish the images** (before or with the
   release commit — the regenerated manifest must be in the tree CI evaluates).
   Publish to the registry projects actually pull from
   (`registry.lan/carbonteq`). That is also what `published.toml` records as
   `default_prefix`. GHCR is optional and unused by current consumers — skip
   it unless you have an explicit reason to stage elsewhere.
   ...
   This builds kind variants concurrently, pushes to the LAN registry, reads
   each digest back rather than predicting it, and regenerates
   `published.toml`. Commit the regenerated manifest.
   ...
   **Base should rarely rebuild.** When the committed base still carries the
   ...
   To stage through GHCR first (optional), pass `--registry ghcr.io/carbonteq-ai`
   and `--default-prefix registry.lan/carbonteq`, then copy digests onto the
   LAN. Prefer publishing straight to `registry.lan/carbonteq` — one push, and
   the manifest prefix matches what consumers resolve.
...
10. **Tag the release** on the merged commit and push the tag. Without it the
    wheels on the index have no traceable source.

    ```bash
    git tag -a v<version> -m "posttrain <version>"
...
- **Do not assume an older LAN tag is the new base.** Confirm the digest
  ...
  passing `--base-image`. Prefer publishing straight to the LAN over staging
  on GHCR.
