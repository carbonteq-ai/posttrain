...
The current candidate selects `trl==1.9.2.post10` from `carbonteq/dev`,
built from `carbonteq-ai/trl` commit
`7a423fd8476e67340fdb912127b37e4b71973588`. Its prerelease tag,
`carbonteq-v1.9.2.post10`, plus wheel and source hashes are recorded in
...
retained a clean-install receipt in workflow `31649996139`. Selected GPU
...
`trl==1.9.2.post10` and still requires qualification for each selected
...
This repair is in immutable `trl==1.9.2.post10` prerelease bytes, tagged at
`carbonteq-v1.9.2.post10`. Posttrain's candidate consumes it from
