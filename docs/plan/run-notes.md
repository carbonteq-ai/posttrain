...
- [ ] Milestone 3: reference language, note components (`chart`, `value`, `table`) and renderer.
...
- Decision: notes can embed data components, a deliberately small version of evidence.dev (Markdown pages that embed live charts and values): three block components, `chart`, `value` and `table`, written as fenced blocks whose language names the component and whose body is a few `key: value` lines, plus a `runs:` selector that names runs or filters them. No query language.
...
Milestone 3 also adds note components: data blocks that the Observatory draws, in the spirit of evidence.dev but with three primitives only. A component is a fenced code block whose language is the component name and whose body is a few `key: value` lines (a list is written `[a, b]`); the body is parsed by the same hand-written parser, never as YAML with anchors or tags, and unknown keys are an error. `runs:` appears in every component and selects runs: `self` (the note's run, the default), explicit run ids, or one filter mapping with any of `work_package`, `job_kind`, `status` and `limit` (newest first) that is resolved through `list_runs`. A `chart` block draws one line chart: `metric:` one metric name or a list, `runs:`, optional `x: step` (default) or `x: time`, `smooth: <n>` (trailing moving average over n points), `title:`. A `value` block shows one large number: `metric:`, `reduce:` (the same reductions as inline references), optional `run:`, `compare:` (another run; the block then also shows the difference), `format:` (the inline filters), `label:`. A `table` block shows a table with one row per selected run and one column per entry of `columns:`, where each entry is either a metric with a reduction (`train/rl/reward_mean last`) or a run path (`run.settings.beta`); an optional `sort:` names a column. For example, a comparison note can contain a `chart` block with `metric: train/rl/entropy` and `runs: [self, lfm26-vortex-v5-150-lr5e5-kl5e3-20260926-r2]`, and a `table` block with `runs: {work_package: train/lfm2.5-1.2b/automationbench-sampo-local-8gb, status: succeeded}` and `columns: [train/rl/reward_mean last, train/step_time_seconds mean, run.settings.beta]`. `parse` returns component nodes with their validated properties; `resolve` fetches their data through `RunDataSource` (`list_runs`, `metric_series`, `get_run`) and returns plain data (series points, a value and difference, table rows), so the Observatory page and the CLI draw from the same resolved result. A component whose metric or run cannot be resolved shows a visible error in place of the chart or value, never an empty chart. Templates in Milestone 4 may use components, so a run card can include, for example, its reward and entropy chart.
...
Milestone 3 is accepted when renderer tests cover every reference form and reduction and each component (`chart` with two runs and two metrics, `value` with `compare`, `table` with a `runs` filter) against fixture runs, a component with an unknown key or metric resolves to a visible error, an unknown path and an unknown metric render the unresolved marker, a reference to another run resolves through the same source, and a body containing `{{__import__("os")}}` renders as unresolved text without evaluation.
...
In `apps/observatory/src/posttrain_observatory/note_render.py`:
    ...
    # Resolved component data, shared by the Observatory page and the CLI.
    class ChartData(TypedDict): title: str | None; x: Literal["step", "time"]; series: list[SeriesPoints]
    class ValueData(TypedDict): label: str; value: float | None; compare: float | None; delta: float | None; format: str | None
    class TableData(TypedDict): columns: list[str]; rows: list[dict[str, object]]
...
Revision note (2026-09-27): removed note authorship (author fields, identity header, required MCP author argument) at the user's request because the platform has no users; notes record only their automatic `source`. Earlier the same day, the Trackio fork's dashboard and MCP server were taken out of scope; the fork provides storage, server endpoints and client methods only. Later the same day, note components (`chart`, `value`, `table`, and a `runs:` selector) were added to Milestone 3, following the user's direction to build a small evidence.dev-style layer.
