...
Revision note (2026-09-27): removed note authorship (author fields, identity header, required MCP author argument) at the user's request because the platform has no users; notes record only their automatic `source`. Earlier the same day, the Trackio fork's dashboard and MCP server were taken out of scope; the fork provides storage, server endpoints and client methods only.
