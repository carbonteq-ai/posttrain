...
Revision 12 — 2026-09-08. Status: the TRL session passes a real vLLM 0.25.1 lifecycle gate, the framework exposes a validated loopback native token endpoint, and native Verifiers worker configuration preserves the selected chat template with LFM token parity. Shared contracts, veRL sampling/renderer propagation, native worker admission, V1 complete-group recovery, and opt-in framework translation are implemented. Changed-weight parity, full worker episode transport, immutable runtime publication, and end-to-end GPU qualification remain open. Repository: `/home/hammad/projects/rl`.
