...
Revision 16 Verifiers synchronization checkpoint: canonical branch `codex/carbonteq-verifiers-latest` is locally synchronized at `36eac9d5e04ef29b584b6fa4f027af00cd76ea19` to upstream main `27bbd216df0af719a43705866b2cf6139bcc95de`. The fork ledger records the retained CarbonTeq delta, Prime-RL's consumer-owned `PolicySpan` boundary, and which staleness/partial-rollout strategies the current evidence and cancellation contracts can support. The complete `tests/v1` suite passes; only credential-dependent Prime cases skip. This local candidate does not change the selected Posttrain pin until it is pushed and the consumer integration gates pass.
