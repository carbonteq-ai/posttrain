...
- [ ] (2026-09-23 14:55Z) Release gate blocked: the shared checkout's `uv.lock` now resolves Trackio dev25 while the runtime profile still pins dev24; `posttrain-release check` fails. A catalog test also finds a stale dependency-lock digest. Do not publish or submit from this dirty state.
