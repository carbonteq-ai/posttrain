...
- [x] (2026-09-25) Environment binding `automationbench-lfm26-train-mix-v7` (committed as `train-mix-v5`; renamed when the release and matched-evals branches were merged, see Decision Log): train-mix-v3 activation plus `task.turn_budget: 12`, source revision acb3043, `max_concurrent: 64`. Only this entry moves; every other environment entry and eval program stays on 9bbd3116. Verifiers environment packages are pinned per catalog entry, not in `uv.lock`.
...
- **Decision:** 16 prompt groups × 4 generations per update.
  ...
  **Date/Author:** 2026-09-24, user.
- **Decision:** Rename this plan's environment binding from `automationbench-lfm26-train-mix-v5` to `automationbench-lfm26-train-mix-v6`, then to `automationbench-lfm26-train-mix-v7`.
  **Rationale:** `codex/vortex-yield-first-release` (57e3797c) already defined `train-mix-v5` as the 8K-per-call, 16K-episode yield-first qualification mix, and recorded run `…yield-first-v4-8k-20260923-r1` under it. The release keeps the id. The v5 runs r4 and `lfm26-vortex-v5-100-from-r4-step20-20260925-r2` were recorded under the old id, and their environment revision acb3043 identifies them unambiguously. VORTEX resumes do not compare environment ids. `codex/vortex-matched-evals` (b2188cd9) had meanwhile pushed its own `train-mix-v6`, the mix-v3 task population that reserves the held-out and difficulty-tiered evaluation sets, so this binding moved on to `train-mix-v7`. It still trains on mix-v2, which includes the 20 tiered evaluation tasks; a descendant that must keep the tiered set held out should start from mix-v3.
