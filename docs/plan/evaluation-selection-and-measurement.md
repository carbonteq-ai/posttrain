...
- [x] 2026-09-15: Before submitting the corrected cells, increased the held-out benchmark output budget to 12,288 tokens for all three arms and expanded the local inference context envelope to 16,384 tokens. No evaluation run was submitted by this configuration change.
- [x] 2026-09-15: Submitted the corrected comparison to dstack on the RTX PRO. Random update-20 completed all 60 repetitions with no execution failures or truncations, but final artifact upload failed because the RustFS VM clock was 3,660 seconds slow. Base `r2`/`r3` and adaptive `r2` failed before evaluation startup because dstack had to install `openssh-server` at container startup while the worker's Debian-mirror route was intermittent.
- [x] 2026-09-15: Restored RustFS clock synchronization, qualified a fresh random-byte artifact through the exact Trackio dev23 client, and made chrony step future material VM drift. Submitted replacement base `r4`, random `r3`, and adaptive `r3` cells. Base `r4` reproduced the external apt dependency with a captured mirror timeout, so the v4 universal runtime now preinstalls `openssh-server`; a republished base/eval/job capsule is required before retrying that failed cell.
- [x] 2026-09-15: Published the hardened v4 runtime graph from framework commit `99f32f915c49d6b8f7fc028711275f4c931b2720`, including base digest `sha256:065c2c31b8d6065fff7ecf81da675eb47756676b4229870ec2876bcdb75b8dfd` and eval digest `sha256:d121851f06436ae891c09ba6c7a52eee1a8dbfcbc1135629047f5feb0f933758`; manifest commit `b559a2f2` records all republished child digests. The actual eval capsule `sha256:ddeaa48021e86b3ca33961654068675f8de88269c83468ef9259c179f36f6e45` crossed dstack bootstrap without a runtime package install.
- [x] 2026-09-15: Completed the three-cell held-out comparison with 20 tasks and three repetitions per task. Base `r5`, random update-20 `r3`, and adaptive update-20 `r4` each reached 60/60 valid repetitions with zero execution failures, retries, truncations, or missing repetitions. Strict task means were 0.3916667, 0.3833333, and 0.3750000 respectively; success frequencies were 0.3833333, 0.3666667, and 0.3500000.
...
Trackio/Doris trace visibility is eventually consistent after the provider process reports success. Base `r5` initially appeared as 55/60 and base `r6` as 53/60 even though the native evaluation artifact and synchronization metrics reported 60 emitted records; `r5` later converged to 60/60 and a complete strict measurement without another write. Release qualification must poll the measurement coverage itself to completeness rather than treating the first terminal-run projection as final or assuming a partial immediate read proves trace loss.
...
Selection, exact dispatch, stable repetition slots, typed task-level measurement, and the first Observatory overview are implemented and tested. The matched LFM2.5/AutomationBench held-out qualification now has complete base, random-curriculum update-20, and adaptive-curriculum update-20 evidence over the same 20-task manifest and three repetitions. At this small sample, adaptive did not improve the held-out task mean: base scored 0.3916667, random 0.3833333, and adaptive 0.3750000. The run also drove durable fixes for runtime activation compatibility, structured reward projection, environment-owned task identity, RustFS clock discipline, and dstack's dependency on runtime Debian downloads. Retry-attempt preservation, seed controls, prepared CLI artifacts, paired comparison, cached projections, and full product views remain pending. Future updates must distinguish tested behavior from proposed interfaces and record exact backend pins and evidence links.
...
The `r1` cells above are diagnostic evidence because the 2,048-token ceiling made their strict estimator partial. The corrected release-gate cells use the same manifest, repetitions, model lineage, and model-recommended sampling parameters with a 12,288-token benchmark output budget:
