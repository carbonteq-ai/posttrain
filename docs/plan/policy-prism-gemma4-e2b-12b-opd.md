...
- Observation: after the XGrammar correction, the first five uniquely named production attempts exercised progressively later boundaries rather than repeating the original schema failure.
...
- Observation: the step-64 checkpoint contains the trainer, optimizer, scheduler, adapter, RNG, and Policy Prism allocation-ledger state needed for exact continuation.
  Evidence: `training-models-gemma4-e2b-it-bf16-distill-checkpoint-step-0064:v0` has Trackio manifest digest `820a1654acffe0f43adabcc93ca95a28767bb6eac1a84352cb5dcf8e24ad3e8e` and downloaded-tree digest `186fc99c524b412f3fbcfe196b15d0cff23b041945a4e4b43de0b0bed28356d7`. The checkpoint reports global step 64 of 384; its ledger passes SQLite integrity and records 64 accepted targets, 71 candidate attempts, and seven reserve claims.
- Observation: Trackio artifact manifest identity and PostTrain materialized content identity are deliberately different digests.
  Evidence: the first pending resume package declared the Trackio manifest digest as `posttrain_content_digest`; a read-only fresh download proved the tree digest differed. The unassigned admission was cancelled before model code or GPU use, the catalog now pins the tree digest used by the Trackio materializer, and the corrected package passed isolated qualification.
...
- Decision: use uniquely prefixed run IDs (`opda01`, `opdsc01`, and `opdrc01`) rather than modify Trackio naming.
  ...
  Date/Author: 2026-08-07 / Codex.
- Decision: consume all two packaged same-stratum reserves for full trajectories by permitting three candidate trajectories and nine total calls.
  Rationale: the reviewed production plan already supplies those reserves. The prior two-candidate/six-call ceiling silently left one unusable and caused a valid graph target to abort even though another compatible source remained.
  Date/Author: 2026-08-07 / Codex.
