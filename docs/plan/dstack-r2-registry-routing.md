...
- [ ] Complete Milestone A cleanup after accepting its successor digest. Delete the rejected `sha256:ad9bd883...` and `sha256:0b56deae...` test manifests and their build tags from both registry endpoints when present, remove their mirror queue/receipt rows only through the controller's exact-digest maintenance path, confirm the qualification Pod is absent, and remove extracted layer diagnostics. Run registry blob garbage collection only offline and only after release-root reachability proves no surviving manifest owns a blob. Preserve `sha256:9bf4ff...`, the accepted successor, every shared reachable blob, and useful BuildKit cache; remove cache records only when they can be attributed exactly to rejected candidates.
- [x] (2026-08-30) Accepted veRL kind image `sha256:1c63c3c3aa5a132ed5643a3b6df686eff5be6283925a8ce1457d413cf9497f8b`. Its first 24 ordered layer descriptors are byte-identical to rollback `sha256:9bf4ff...`; only the intended final compatibility/declaration layer changes, to `sha256:5ab237...` at 104,220,061 compressed bytes. The published manifest now pins the accepted digest. Actual-job packing, same-digest local/RunPod qualification, and cleanup remain open.
