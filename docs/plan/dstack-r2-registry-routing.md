...
- Observation: the shared job-kind Bake file was an accidental cross-variant invalidation boundary for the dedicated veRL Dockerfile.
  Evidence: adding veRL-only label arguments temporarily made the publisher plan builds for every kind. The invocation was interrupted while its first unrelated image exports were being cancelled, and the committed manifest remained unchanged. The publisher now selects `verl-py313/docker-bake.hcl` for veRL and excludes the shared Bake file from veRL's source digest. The read-only plan again reports zero-transfer reuse for the base and all five unrelated kinds, with only veRL marked `build`.

- Observation: the remaining 1.55 GB veRL dependency-layer drift came from an unnamespaced wheel-file collision, not package versions, compression, or timestamps.
  Evidence: extracted comparison of rollback `sha256:9bf4ff...` and rejected candidate `sha256:0b56deae...` found exactly two changed paths among 93,627 regular files: `site-packages/build_backend.py` and its hash-based bytecode. FlashInfer and `torch-c-dlpack-ext` both claim that top-level filename, and uv may install independent wheels concurrently. The image now restores `build_backend.py` from the exact SHA-256-pinned `torch-c-dlpack-ext` wheel before deterministic compilation; its bytes match the qualified rollback layer.

- Observation: Docker ARG placement is part of the cache boundary even when the value is used only by a later instruction.
  Evidence: candidate `sha256:86b4ec9d...` made the 1.55 GB dependency layer byte-identical but changed the earlier 19 MB apt layer only because new wheel ARGs preceded it. Extracted comparison found one changed file, `var/log/alternatives.log`, whose only difference was the invocation timestamp. The wheel ARGs now begin immediately before the dependency step that consumes them, retaining the qualified apt cache rather than manufacturing a historical log timestamp or deleting inherited content.

- Observation: valid immutable direct references may include PEP 508 environment markers.
  Evidence: actual-job packing initially rejected the veRL backend constraints even though Verifiers, veRL, and vLLM each use full 40-character Git commits. The validator incorrectly required the commit to end the entire line and did not admit `; sys_platform == 'linux'`. It now accepts a marker only after a full commit while continuing to reject branch names and truncated revisions; 37 focused dependency tests pass and `posttrain doctor` accepts the profile.

- Observation: the existing sampling canary correctly failed closed when its immutable training selection named an older backend lock than the accepted kind image.
  Evidence: selection `training/qwen3.5-0.8b-verl-grpo-sampling-contract-canary-lora@2` records lock `334831a7...`, while the accepted kind records `19f96d07...`. The historical selection remains unchanged. New revision `@3` preserves the same source, runtime, and sampling behavior while naming the accepted lock, and only the qualification work package advances to it. Catalog validation reports 71 base and 95 project entries; its focused suite passes `12 passed, 1 skipped`.

- Observation: OCI descriptor annotations are not a reliable cache-identity comparison across an actual-job repack.
  Evidence: BuildKit omitted `buildkit/rewritten-timestamp` annotations from inherited kind descriptors while preserving every inherited digest, size, media type, and order. The content descriptors are identical and Docker reused all parent blobs; the comparison gate now records normalized content descriptors separately from optional producer annotations.

- Observation: safe cache cleanup requires stronger ownership evidence than age, size, or build proximity.
  Evidence: the two recent large BuildKit records map to accepted kind `sha256:1c63c3c...`, older multi-gigabyte records belong to unrelated TRL images, and no cache record uniquely names a rejected manifest. Offline registry reachability could prove 46 LAN and 21 R2 blobs unowned, but BuildKit could not; registry GC proceeded while shared build cache was preserved.
...
- Decision: Use immutable digests and stable logical IDs for identity and cleanup; timestamps are observation metadata only.
  Rationale: image, publication, run, fleet, Pod, and cleanup ownership must survive retries and must be addressable exactly. The qualification wrapper therefore uses stable run/fleet names and digest-pinned images, while receipt timestamps record when an observation occurred but never select an image, cache entry, registry object, or provider resource for deletion.
  Date/Author: 2026-08-30 / Codex and user
...
After acceptance, inventory test publications by exact repository, tag, and digest before deleting anything. Remove rejected manifests and tags from the LAN registry and public mirror, reconcile only their exact controller records, and then recompute reachability from all retained release roots. Blob garbage collection is permitted only while the affected registry is offline and only when that reachability proof shows the blobs are unowned. Inspect BuildKit disk usage before cache cleanup; retain shared cache and remove only records uniquely attributable to rejected candidates. Delete bounded extraction directories and qualification state after their safe receipt has been retained. The cleanup result must list removed identities, preserved roots, reclaimed bytes when measurable, and confirmed provider absence.
...
2026-08-30: Completed Production Milestone A with one digest-pinned actual-job
image on local and RunPod CUDA, a protected successful-canary receipt, and
confirmed provider absence. Added an exact authenticated controller maintenance
delete for rejected publication rows, removed the three rejected candidates by
repository/digest, and ran both registries' garbage collectors offline after
reachability checks. Shared BuildKit records remain because none could be
