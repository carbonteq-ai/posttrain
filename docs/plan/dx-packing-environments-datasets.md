...
- [x] (2026-08-01) Milestone 1: named activation resources with generated
      locks. `verifiers-config` activations now declare project-relative files;
...
- Observation: the activation's semantic digest must remain a digest of the
  declared source, not the materialized absolute path.
  Evidence: the resource lock retains its declared relative path for the
  activation digest and records separate staged path/content facts for the
  package; the worker creates its capsule-local path only after verification.
...
- Milestone 1 outcome: a project data file can no longer accidentally be a
