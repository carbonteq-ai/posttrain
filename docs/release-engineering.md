...
required Quality, allocates the next unused PEP 440 `X.Y.ZrcN`, publishes that
RC only to `carbonteq/dev`, qualifies the final-identity OCI digests plus one
real packed job, and retains the wheelhouse and evidence. Every RC is immutable;
a failed attempt consumes its RC number and the next attempt allocates another.

**Publish release** runs only after a candidate passed and the release PR
merged. It verifies that the candidate and merged trees are identical or differ
only in allowlisted release plumbing, rechecks the RC in development, restores
the accepted runtime manifest and locks, and builds final `X.Y.Z` Python
metadata from that materialization. A promotion receipt binds the RC and final
receipts and requires identical package sets and runtime-manifest digests. The
final artifacts publish directly to `carbonteq/stable`, are clean-installed,
and are tagged last. The bounded GPU job is not repeated.
...
for a different source or configuration. The candidate is a real PEP 440
`rcN` distribution. Promotion never renames that wheel; it performs one
controlled final-metadata build from the accepted source and generated inputs.
...
| 7 | Attested final-metadata build published to stable | Make the accepted materialization consumable as `X.Y.Z` without renaming RC files |
...
3. uses the authored `X.Y.Z` from `release/manifest.toml` to allocate the next
   unused immutable `X.Y.ZrcN` development candidate;
...
A failed candidate never reaches stable and never gets overwritten; another
attempt allocates the next RC. After one candidate passes, its accepted
materialization is retained and the release PR merges. The final transaction:
...
2. verifies the retained RC and development-index hashes;
3. rebuilds final-version Python metadata from the accepted source, runtime
   locks, and OCI manifest, then writes a promotion receipt binding both
   distribution receipts to the merged tag target;
4. publishes the final files directly to `carbonteq/stable`, verifies readback,
   and proves a clean stable-index install;
...
installed from the development index. Never overwrite an RC, replace a stable
version, or reinterpret an OCI digest. A repair before final publication
allocates the next RC under the same authored target. A repair after stable
