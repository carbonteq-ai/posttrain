...
class Architecture:
    ...
    def from_config(cls, config: Mapping[str, Any], parameters: int) -> Architecture:
        ...
        if types:
            ...
            sliding = sum(kind in {"sliding_attention", "local_attention", "sliding_window_attention"} for kind in types)
...
def suggest(hardware: Hardware, architecture: Architecture, task: Task, *, weight_bytes: int = 2) -> Suggestion:
    ...
    if native is not None and context > native:
        ...
        context = native
    settings.append(Setting("max_model_len", context, "prompt plus completion budget, rounded to 256; shorter contexts fail long episodes outright (4% of LFM2.5 AutomationBench episodes at 24,576)"))
    ...
    if turboquant:
        ...
        settings.append(Setting("dtype", dtype, "bf16 checkpoints must compute in bf16 where the GPU supports it; float16's narrower exponent can overflow and shifts logprobs away from training"))
    ...
    settings.append(
        Setting(
            ...
            + (f", capped at the {max_concurrency} typical-length sequences that fit" if in_flight < task.concurrency else "")
    ...
    settings.append(Setting("enable_prefix_caching", True, "multi-turn and grouped prompts share context; 80-89% of prompt tokens were cache hits on the LFM2.5 replay; policy updates reset it"))
    ...
    if facts is not None:
        settings.append(
            Setting(
                ...
                "SM120 supports FlashAttention 2 in native vLLM (FA4 is rejected)" if facts.compute_capability[0] == 12 else f"best native FlashAttention for compute capability {facts.compute_capability[0]}.{facts.compute_capability[1]}",
            )
        )
    ...
    if task.purpose == "rollout":
        settings.append(Setting("mode", "colocate" if trainer else "server", "colocated with the trainer" if trainer else "dedicated rollout server"))
        if trainer:
            settings.append(Setting("sleep_during_optimization", True, "release rollout memory to the optimizer between collections"))
    if task.lora_rank:
    ...
    if task.reproducible_logprobs:
        ...
        notes.append("batch invariance costs 2-6% decode throughput on the CarbonTeq fork (Gemma-4-12B 2.0%, E4B 5.9% at c4)")
    ...
    memory = {
        ...
        **({"other_engines": round(others, 2)} if others else {}),
        "free": round(hardware.memory_gb - shared - weights - draft_weights - workspace - (0 if eager else graphs) - kv_gb, 2),
    }
    bound = None
...
def _step_capacity(task: Task, fits: int | None, read_weights: float, per_sequence: float) -> StepCapacity | None:
    ...
    if not step or not group or not fits:
        return None
    knee = math.ceil(THROUGHPUT_KNEE / (1 - THROUGHPUT_KNEE) * read_weights / per_sequence) if per_sequence > 0 else fits
    ...
    if margin < group:
        reason = (
            f"the step's {step} rows already use the engine's useful concurrency ({useful})"
    ...
    def cost(rows: int) -> float:
        ...
        return full * (read_weights + fits * per_sequence) + (read_weights + last * per_sequence if last else 0.0) if waves_needed else 0.0
