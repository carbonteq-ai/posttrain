...
QUALIFIED_DRAFTERS: Mapping[str, tuple[tuple[str, str, str], ...]] = {
    "google/gemma-4-12B-it": (
        (
            ...
            "MTP-2 raised Gemma 4 12B decode from 181.4 to 253.9 tok/s (+40%) at concurrency 4",
        ),
        ("dspark", "deepseek-ai/dspark_gemma4_12b_block7", "a DSpark block drafter is qualified for Gemma 4 12B"),
    ),
    "google/gemma-4-31B-it": (
        ("mtp", "google/gemma-4-31B-it-assistant", "a paired MTP assistant is published for this checkpoint"),
    ),
    "google/gemma-4-E4B-it": (
        ("mtp", "google/gemma-4-E4B-it-assistant", "a paired MTP assistant is published for this checkpoint"),
    ),
    "google/gemma-4-E2B-it": (
        ("mtp", "google/gemma-4-E2B-it-assistant", "a paired MTP assistant is published for this checkpoint"),
    ),
    "Nanbeige/Nanbeige4.2-3B": (
        ("dspark", "Nanbeige/Nanbeige4.2-3B-DSpark", "a DSpark drafter is qualified for Nanbeige 4.2 3B"),
    ),
}
...
def _issue(
    ...
    if severity in {"error", "warning"} and isinstance(reason, str) and reason.strip():
        return ConfigurationIssue(
            code, "info", "static", seat.role, path, f"{message} Acknowledged by {seat.label}: {reason}", None, related
        )
    if severity == "error":
...
def _binding_findings(snapshot: Snapshot, seat: Seat) -> Iterator[ConfigurationIssue]:
    ...
    if accelerator in _SM120 and flash == 4:
        ...
        yield _issue(
            seat,
            "FLASH_ATTN_VERSION_UNSUPPORTED_ARCH",
            "warning",
            f"{role}.engine.flash_attn_version",
    ...
    if family == "gemma4" and "SM120_FA4" in priority:
        yield _issue(
            seat,
            "VLLM_SM120_FA4_ON_GEMMA",
            "error",
            f"{role}.engine.attention_backend_priority",
    ...
    if len(priority) > 1:
        yield _issue(
            seat,
            "ATTENTION_BACKEND_PRIORITY_TRUNCATED",
            "warning",
            f"{role}.engine.attention_backend_priority",
    ...
    if (
        ...
        yield _issue(
            seat,
            "VLLM_HYBRID_CONTINUATION_REUSE_MISSING",
            "error" if busy else "warning",
            f"{role}.backend",
    ...
    if turboquant and long_qwen:
        yield _issue(
            seat,
            "TURBOQUANT_QUALITY_UNQUALIFIED",
            "warning",
            f"{role}.engine.kv_cache_dtype",
    ...
    if backend == _PINNED_DSPARK_BACKEND and family == "nanbeige4.2" and turboquant and method == "dspark":
        yield _issue(
            seat,
    ...
    if (
        method == "mtp"
        and capabilities.get("mtp") is False
        and speculative is not None
        and "draft_model" not in speculative
    ):
        yield _issue(
            seat,
    ...
    if (
        purposes & {"rollout", "eval", "screen", "judge", "smoke"}
        and speculative is None
        and hardware.get("supports_mtp") is not False
    ):
    ...
    if evaluates and not invariant and max_num_seqs != 1:
        yield _issue(
            seat,
            "BATCH_INVARIANCE_OFF_FOR_EVALUATION",
            "warning",
            f"{role}.engine.batch_invariant",
    ...
    if invariant and family in _INVARIANCE_UNSUPPORTED:
        yield _issue(
            seat,
            "BATCH_INVARIANCE_UNVALIDATED_ARCH",
            "error",
            f"{role}.engine.batch_invariant",
    ...
    if invariant and (flash == 4 or "SM120_FA4" in priority):
        yield _issue(
            seat,
            "BATCH_INVARIANCE_WITH_SM120_FA4",
            "error",
            f"{role}.engine.batch_invariant",
...
def _training_findings(snapshot: Snapshot) -> Iterator[ConfigurationIssue]:
    ...
    rollout = next(
        (seat for seat in inference_seats(snapshot) if "rollout" in items(seat.resolved.get("purpose"))), None
    )
    ...
    if backend == "trl" and rollout is not None:
        ...
        if colocated and engine.get("sleep_during_optimization") is not True:
            yield issue(
                ...
                "warning",
                f"{role}.engine.sleep_during_optimization",
        ...
        if execution:
            ...
            if workers and episodes and limit is not None and limit < workers * episodes:
                problems.append(
                    f"the environment's max_concurrent {limit} is below {workers} x {episodes} worker slots"
                )
            if problems:
                yield issue(
                    "TRL_ROLLOUT_EXECUTION_INVALID",
                    "error",
                    f"{training.role}.backend_options.rollout_execution",
                    "native rollout workers are configured but "
                    + "; ".join(problems)
            ...
            yield issue(
                "TRL_ROLLOUT_BATCH_REQUEST_MODE",
                "recommendation",
                f"{role}.engine.request_mode",
        ...
        if (
            ...
            and speculative.get("method") == "uno"
            and (update.get("kind") not in {"lora", "qlora"} or engine.get("weight_sync_mode") != "lora")
        ):
            yield issue(
                "TRL_UNO_REQUIRES_LORA_SYNC",
                "error",
                f"{role}.engine.weight_sync_mode",
        ...
        if options.get("use_liger_kernel") is True and job in {"train.gdpo", "train.capo"}:
            yield issue(
                "TRL_LIGER_UNQUALIFIED_FOR_ALGORITHM",
                "error",
                f"{training.role}.backend_options.use_liger_kernel",
        ...
        if update.get("kind") == "qlora" and engine.get("weight_sync_mode") != "lora":
            yield issue(
                "QLORA_REQUIRES_LORA_WEIGHT_SYNC",
                "error",
                f"{role}.engine.weight_sync_mode",
        ...
        if engine.get("weight_sync_mode") == "lora" and (
            ...
            yield issue(
                "WEIGHT_NAME_PREFIX_FAMILY_MISMATCH",
                "error",
                f"{role}.engine.weight_name_prefix",
