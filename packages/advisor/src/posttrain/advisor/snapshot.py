...
class Seat:
    ...
    def label(self) -> str:
        return (
            f"{self.selection_id}@{self.revision}"
            if self.revision and "@" not in self.selection_id
            else self.selection_id
        )
...
def served_model(snapshot: Snapshot, seat: Seat) -> Mapping[str, Any]:
    ...
    facts: dict[str, Any] = {
        key: model.resolved[key]
        for key in ("family", "weight_precision", "parameters", "capabilities")
        if key in model.resolved
    }
