...
def binding_task(
    snapshot: Snapshot, seat: Seat, architecture: Architecture, draft: Architecture | None = None
) -> Task:
    ...
    prompt_limit, completion_limit = integer(values.get("max_prompt_length")), integer(values.get("max_completion_length"))
    if purpose == "rollout" and prompts and generations and prompt_limit and completion_limit:
        ...
        concurrency = (
            integer(environment.resolved.get("max_concurrent")) if purpose == "eval" and environment is not None else None
        ) or integer(engine.get("max_num_seqs")) or 8
...
def _co_tenant_gb(snapshot: Snapshot, seat: Seat, memory_gb: float, load_architecture: ArchitectureLoader) -> float:
    ...
    colocated_rollout = next(
        (other for other in inference_seats(snapshot) if _trl_colocated(snapshot, other) or (
            "rollout" in items(other.resolved.get("purpose")) and mapping(other.resolved.get("engine")).get("mode") == "colocate"
        )),
        None,
    )
    if (
        ...
        if isinstance(base.get("repo_id"), str):
            policy = load_architecture(base["repo_id"], base.get("revision") if isinstance(base.get("revision"), str) else None)
            settings = settings_seat(snapshot)
...
def recommend(snapshot: Snapshot, seat: Seat, load_architecture: ArchitectureLoader) -> dict[str, JsonValue]:
    ...
    if isinstance(draft_model.get("repo_id"), str):
        ...
        try:
            draft = load_architecture(draft_model["repo_id"], draft_revision if isinstance(draft_revision, str) else None)
