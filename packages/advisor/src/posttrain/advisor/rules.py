...
def _performance_issue(
    ...
    if isinstance(reason, str) and reason.strip():
        return ConfigurationIssue(
            code, "info", "static", seat.role, path, f"{message} Acknowledged by {seat.label}: {reason}", None, related_paths
        )
    return ConfigurationIssue(
...
def performance_findings(snapshot: Snapshot) -> tuple[ConfigurationIssue, ...]:
    ...
    if settings is not None:
        prompt, completion = (integer(settings.resolved.get(key)) for key in ("max_prompt_length", "max_completion_length"))
...
def training_findings(snapshot: Snapshot) -> tuple[ConfigurationIssue, ...]:
    ...
    if settings.resolved.get("lr_scheduler_type") == "linear" and max_steps is not None and max_steps <= _SHORT_RUN_STEPS:
        issues.append(
            ConfigurationIssue(
