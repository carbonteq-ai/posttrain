...
def test_old_runtime_misses_hybrid_prefix_reuse_for_multi_turn_episodes(snap) -> None:
    ...
    seat = _seat(snap, {"enable_prefix_caching": True, "max_num_seqs": 32, "batch_invariant": True}, backend="vllm@0.25.1", model=lfm)
...
def test_evaluations_without_batch_invariance_warn_and_invariance_needs_the_tuned_runtime(snap) -> None:
    ...
    assert _codes({"evaluation_inference": _seat(snap, invariant, backend="vllm@0.25.1")})["BATCH_INVARIANCE_UNTUNED_RUNTIME"] == "error"
...
def test_models_with_qualified_drafters_warn_when_speculation_is_off(snap) -> None:
    ...
    (issue,) = [issue for issue in rule_findings({"judge": _seat(snap, engine, purpose=("judge",), model=gemma)}) if issue.code == "SPECULATIVE_DECODING_AVAILABLE"]
    ...
    assert _codes({"judge": _seat(snap, engine, purpose=("judge",), model=qwen)})["SPECULATIVE_DECODING_AVAILABLE"] == "warning"
    with_mtp = {**engine, "speculative_config": {"method": "mtp", "num_speculative_tokens": 2}}
    assert "SPECULATIVE_DECODING_AVAILABLE" not in _codes({"judge": _seat(snap, with_mtp, purpose=("judge",), model=qwen)})
...
def test_attention_backend_rules(snap) -> None:
    gemma = _served("gemma4", "google/gemma-4-12B-it")
    engine = {"enable_prefix_caching": True, "batch_invariant": True, "speculative_config": {"method": "mtp"}, "attention_backend_priority": ["SM120_FA4", "TRITON_ATTN"]}
    ...
    fa4 = {"enable_prefix_caching": True, "flash_attn_version": 4, "kv_cache_dtype": "turboquant_k8v4", "batch_invariant": True}
...
def test_trl_rollout_rules(snap) -> None:
    engine = {**snap.TUNED, "request_mode": "batch", "dtype": "bfloat16", "weight_sync_mode": "lora", "weight_name_prefix": "model."}
    ...
    codes = _codes({"training": training, "environment": snap.environment(max_concurrent=32), "rollout_inference": rollout})
...
def test_acknowledged_compatibility_findings_become_information(snap) -> None:
    ...
    seat["resolved"]["performance_acknowledgements"] = {"BATCH_INVARIANCE_OFF_FOR_EVALUATION": "scores are compared only within one run"}
