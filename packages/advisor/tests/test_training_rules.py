...
def test_vortex_v1_settings_warn_about_rate_and_schedule(snap) -> None:
    ...
    assert codes == {"LORA_RL_LEARNING_RATE_BELOW_REFERENCE": "warning", "LR_SCHEDULE_DECAYS_WITHIN_SHORT_RUN": "warning"}
...
def test_reference_range_scales_with_alpha(snap) -> None:
    ...
    assert "LORA_RL_LEARNING_RATE_BELOW_REFERENCE" in _codes({"settings": snap.settings(5e-6), "training": snap.training(alpha=32)})
    assert _codes({"settings": snap.settings(1e-3), "training": snap.training()}) == {"LORA_RL_LEARNING_RATE_ABOVE_REFERENCE": "warning"}
