...
def test_plan_derives_project_environment_tree_identity_without_git(tmp_path) -> None:
    ...
    expected_digest = ImmutableSourceSnapshotter(cache_root=(tmp_path / "cache").resolve()).inspect(
        SourceSnapshotRequest(root=package.resolve(), includes=(".",), install_roots=(".",))
    )
    ...
    assert first.spec.project_environment_sources[0].tree_digest == expected_digest
