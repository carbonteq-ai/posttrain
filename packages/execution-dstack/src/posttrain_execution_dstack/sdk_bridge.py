...
def _configuration(payload):
    ...
    runtime_secret_references = configuration.pop("_posttrain_runtime_secret_refs", {})
    ...
    if isinstance(environment, list):
        ...
        configuration["env"] = {
            name: (
                "${{ secrets." + runtime_secret_references[name] + " }}"
                if name in runtime_secret_references
                else runtime_environment[name]
            )
