...
def register(app: typer.Typer) -> None:
    ...
    def run_list_cmd(
        ...
        # Stable two-pass ordering keeps lifecycle priority while showing the
        # newest entries first within each state.
        payload.sort(
            key=lambda item: (
        ...
        lines = [
            (
                ...
                f"submitted={item['submitted_at'] or '-'}"
            )
            for item in payload
        ]
    ...
    def run_wait_cmd(
        ...
        state: CliState = ctx.obj
        record = execution_service_for_run(state.layout(), run_id).wait(
            run_id,
    ...
    def run_logs_cmd(
        ...
        state: CliState = ctx.obj
        page = execution_service_for_run(state.layout(), run_id).logs(
            run_id,
            LogCursor(offset),
            limit=limit,
        )
        payload = {
            "run_id": run_id,
            "lines": page.lines,
            "next_offset": page.next_cursor.offset,
    ...
    def run_retry_submit_cmd(
        ...
        state: CliState = ctx.obj
        result = execution_admission_service(state.layout()).retry_submission(run_id)
        emit(
            state,
            {
                "run_id": run_id,
