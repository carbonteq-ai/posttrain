...
SNAPSHOT = {
    ...
    "training": {
        ...
        "resolved": {"backend": "trl@1.12.0", "parameter_update": {"kind": "lora", "rank": 4, "alpha": 8, "target_modules": "all-linear"}},
    },
    "rollout_inference": {
    ...
    "execution_targets": {
        "targets": [{"selection_id": TARGET, "memory_gb": 96.0, "hardware": {"accelerator_model": "RTXPRO6000", "accelerator_count": 1}}]
    },
}


def _series(name: str, values: list[float]) -> MetricSeries:
    return MetricSeries(name=name, points=tuple(MetricPoint(value=value, step=index) for index, value in enumerate(values)))
...
def test_async_review_uses_metric_series_for_calibration() -> None:
    review = asyncio.run(
        configuration_review(SNAPSHOT, None, (_series("train/rl/time/rollout_seconds", [100.0]),))
    )
