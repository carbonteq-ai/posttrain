...
def audit(root):
    ...
    rejected = {
        group: [{"trace_id": record["id"], "stop_condition": record["stop_condition"]} for record in records]
    ...
    for group, records in groups.items():
        ...
        for record in records:
            ...
            scores.append(
                [
                    math.fsum(item["components"][column]["value"] for item in panel["assessments"])
                    / len(panel["assessments"])
                    for column in (0, 1)
                ]
            )
            values, turn = [], 0
        ...
        if algorithm == "gdpo":
            ...
            combined = [
                sum(
                    weight * column[index]
                    for weight, column in zip(settings["component_weights"], columns, strict=True)
                )
        ...
        for name, expected in (("mean", math.fsum(tokens) / len(tokens)), ("min", min(tokens)), ("max", max(tokens))):
            assert math.isclose(actual[f"critic/advantages/{name}"], expected, rel_tol=2e-5, abs_tol=2e-6), (
                step,
                name,
                actual[f"critic/advantages/{name}"],
                expected,
            )
    ...
    return {
        "algorithm": algorithm,
        "updates": sorted(checked),
        "input_audit": inputs,
        "rejected_groups": rejected,
        "gradient_norms": [metrics[step]["actor/grad_norm"] for step in sorted(metrics)],
        "advantage_aggregate_oracle": "passed",
        "checkpoint_resume_qualified": False,
        "per_token_update_qualified": False,
