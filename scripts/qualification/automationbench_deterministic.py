...
def main():
    ...
    if args.backend == "verl":
        ...
        options = {"python_executable": sys.executable, "working_directory": str(source),
                   "source_revision": revision, "attention_implementation": "sdpa"}
        (output / "verl-source.diff").write_bytes(subprocess.check_output(["git", "diff", "HEAD"], cwd=source))
    training = TrainingBinding(
        f"training/deterministic-{args.backend}", "1", backend,
        TrainingRenderer("renderers/qualification", "qwen3.5", "qwen3.5", "off"),
        LoRAUpdate(rank=4, alpha=8), target,
    ...
    if args.backend == "verl":
        engine = {"max_model_len": 8192, "gpu_memory_utilization": 0.4, "enforce_eager": True,
                  "text_only": True, "max_num_seqs": 2, "max_num_batched_tokens": 8192}
    inference = InferenceBinding(
        "inference/deterministic-policy", "1", policy,
        "vllm@0.25.2.dev2" if args.backend == "verl" else "transformers@5.14.1",
        policy.renderer_contract, engine, sampling, target, ("rollout",), capabilities=("tool-calling",),
    )
    loop = TrainingLoop(max_steps=5, max_length=8192, per_device_batch_size=1,
                        gradient_accumulation_steps=2, learning_rate=1e-5, checkpoint_steps=2, checkpoint_limit=3)
    ...
    settings = (GDPOSettings(**extra, component_names=("fixture_a", "fixture_b"), component_weights=(1., 0.5))
                if args.algorithm == "gdpo" else CAPOSettings(**extra))
    projection = RewardProjection(
        "qualification/deterministic-turns", "1",
        ...
        scorer_digest=SCORER_DIGEST, turns_info_key=ANNOTATION_KEY,
        turn_error_key="erroneous_turn_ids" if args.algorithm == "capo" else None,
    )
    context = RunContext("qualification", "automationbench-deterministic", output.name,
                         f"train.{args.algorithm}", "1", output, FileObserver(output / "observations.jsonl"))
    request = build_verifiers_structured_request(
        policy=policy, environment=environment, settings=settings, reward_projection=projection,
        training=training, inference=inference, trace_path=output / "traces.jsonl", run_id=output.name,
    )
    request.bridge.enrichers = (DeterministicTurnFixture(),)
    selection = {"policy": plain(policy), "settings": plain(settings), "training": plain(training),
                 "inference": plain(inference), "environment": plain(environment), "projection": plain(projection),
                 "fixture": FIXTURE, "candidate_unpublished": True,
                 "script_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
    (output / "selection.json").write_text(json.dumps(selection, default=str, indent=2))
    (output / "fixture-source.py").write_bytes(Path(sys.modules[DeterministicTurnFixture.__module__].__file__).read_bytes())
